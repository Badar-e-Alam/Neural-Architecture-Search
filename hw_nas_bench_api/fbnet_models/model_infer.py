...
OPS = {
    "k3_e1": lambda C_in, C_out, layer_id, stride: ConvBlock(
        C_in, C_out, layer_id, expansion=1, kernel_size=3, stride=stride, groups=1
    ),
    "k3_e1_g2": lambda C_in, C_out, layer_id, stride: ConvBlock(
        C_in, C_out, layer_id, expansion=1, kernel_size=3, stride=stride, groups=2
    ),
    "k3_e3": lambda C_in, C_out, layer_id, stride: ConvBlock(
        C_in, C_out, layer_id, expansion=3, kernel_size=3, stride=stride, groups=1
    ),
    "k3_e6": lambda C_in, C_out, layer_id, stride: ConvBlock(
        C_in, C_out, layer_id, expansion=6, kernel_size=3, stride=stride, groups=1
    ),
    "k5_e1": lambda C_in, C_out, layer_id, stride: ConvBlock(
        C_in, C_out, layer_id, expansion=1, kernel_size=5, stride=stride, groups=1
    ),
    "k5_e1_g2": lambda C_in, C_out, layer_id, stride: ConvBlock(
        C_in, C_out, layer_id, expansion=1, kernel_size=5, stride=stride, groups=2
    ),
    "k5_e3": lambda C_in, C_out, layer_id, stride: ConvBlock(
        C_in, C_out, layer_id, expansion=3, kernel_size=5, stride=stride, groups=1
    ),
    "k5_e6": lambda C_in, C_out, layer_id, stride: ConvBlock(
        C_in, C_out, layer_id, expansion=6, kernel_size=5, stride=stride, groups=1
    ),
    "skip": lambda C_in, C_out, layer_id, stride: Skip(C_in, C_out, layer_id, stride),
}

AUX_OPS = {
    "ConvNorm": lambda C_in, C_out, layer_id, kernel_size, stride: ConvBlock(
        C_in, C_out, layer_id, kernel_size=kernel_size, stride=stride
    ),
...
class FBNet_Infer(nn.Module):
    def __init__(self, config):
        ...
        assert (
            len(self.op_idx_list) == 22
        ), "The length of op_idx_list should be 22, while it is {}".format(
            len(self.op_idx_list)
        )
        ...
        if "cifar" in config["dataset"]:
            self.stride_list = [
                ...
                2,
                1,
            ]  # FBNetv1 setting, unoffcial CIFAR-10/100 setting
        else:
            self.stride_list = [
                1,
                2,
                ...
                2,
                1,
            ]  # FBNetv1 setting, offcial ImageNet setting
            ...
            for i in range(num_layer):
                if i == 0:  # first Conv takes the stride into consideration
                    if stage_id == 0:
                        ...
                        op = OPS[PRIMITIVES[self.op_idx_list[layer_id - 1]]](
                            self.stem_channel,
                            self.num_channel_list[stage_id],
                            layer_id,
                            self.stride_list[stage_id],
                        )
                    else:
                        op = OPS[PRIMITIVES[self.op_idx_list[layer_id - 1]]](
                            self.num_channel_list[stage_id - 1],
                            self.num_channel_list[stage_id],
                            layer_id,
                            self.stride_list[stage_id],
                        )
        ...
        self.header = ConvNorm(
            self.num_channel_list[-1], self.header_channel, kernel_size=1
        )
