        ...
        if self.search_space == "nasbench201":
            ...
            for metric_name in metrics_list:
                results[metric_name] = float(
                    self.HW_metrics[self.search_space][dataname][metric_name][
                        arch_index
                    ]
                )

            # for 'average_hw_metric'
            results["average_hw_metric"] = (
                results["edgegpu_latency"]
                * results["edgegpu_energy"]
                * results["raspi4_latency"]
                * results["edgetpu_latency"]
                * results["pixel3_latency"]
                * results["eyeriss_latency"]
                * results["eyeriss_energy"]
                * results["fpga_latency"]
                * results["fpga_energy"]
            )
            ...
            for metric_name in metrics_list:
                ...
                results[metric_name] = fbnet_get_metrics(
                    arch_index, dataname, lookup_table
                )
            ...
            print(
                "Wrong dataset name, expect: {} and {}, while receive: {}",
    ...
    def get_net_config(self, arch_index, dataname):
        if self.search_space == "nasbench201":
            ...
            assert dataname in [
                "cifar100",
                "ImageNet",
            ], "Only {} and {} are allowed to be datasets in FBNet space, while receive {}".format(
                "cifar100", "ImageNet", dataname
            )
...
def fbnet_get_metrics(arch_index, dataname, lookup_table):
    ...
    assert (
        type(arch_index) == list
    ), "The type of arch_index should be a list in FBNet space, while receive {}".format(
        type(arch_index)
    )
    assert (
        len(arch_index) == 22
    ), "The length of arch_index should be 22 in FBNet space, while receive {}".format(
        len(arch_index)
    )
    assert dataname in [
        "cifar100",
    ...
    metric += OP_metrics_dict[
        "ConvNorm_H{}_W{}_Cin{}_Cout{}_kernel{}_stride{}_group{}".format(
            H_W,
            H_W,
            3,
            stem_channel,
            ...
            if i == 0:  # first Conv takes the stride into consideration
                if stage_id == 0:
                    ...
                    if layer_op_idx < 8:  # ConvBlock
                        ...
                        metric += OP_metrics_dict[
                            "Skip_H{}_W{}_Cin{}_Cout{}_stride{}".format(
                                H_W,
                                H_W,
                                stem_channel,
                                num_channel_list[stage_id],
    ...
    # Add AvgP cost
    metric += OP_metrics_dict[
        "AvgP_H{}_W{}_Cin{}_Cout{}_kernel{}_stride{}".format(
            H_W,
            H_W,
            header_channel,
            header_channel,
